...
| M12.1 · Unit lifecycle CRUD (create/update/state/delete) | ✅ | ✅ | ⬜ | ➖ | ➖ | ⬜ | **Designed, not yet built.** Depends on M12.0. Generalizes unit creation beyond `createChildOrg`; adds `updateUnit`, `setUnitState` (archive/suspend/reactivate, reusing `Unit.State`'s existing enum), and `deleteUnit` (soft-delete, orphan-protected against children, active role assignments, and an existing `religion_org_profiles` row — composed in `internal/core`, since `D-InProcessAuthz` forbids `internal/directory` importing `internal/authz`). Root-unit hard-guarded against state changes and delete. Every mutation audit-logged, matching `GrantUnitRole`'s existing pattern. |
