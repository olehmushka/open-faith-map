...
| U14 | **Resolved at M12.2 (2026-08-24).** `platform-moderator`'s half was resolved at M12.0 (permission split, `moderation.standing`, not a scope narrowing). `registration-operator`'s `unit.edges.manage` half — every real/test grant was hardcoded `scope='unit'`, never `subtree`, so root-scoped `unit.edges.manage` couldn't reach any non-root target — is now resolved by building real `scope="subtree"` grant provisioning (`GrantUnitRoleRequest`/`BulkGrantUnitRoleRequest`'s new `scope`/`graphId` fields, `internal/authz/adapters/store.go` no longer hardcoding `'unit'`), not by narrowing `MoveUnit`'s dual-parent check to `scope='unit'`-only reach. See D-UnitMoveDualScope's M12.2 addendum in `decisions.md` and `TestMoveUnitIntegration` for the direct proof. | `M12.2` |
...
| M11.1 · Account status enforcement + deactivate/reactivate UI | ✅ | ✅ | ✅ | ➖ | ✅ | ✅ | **Built and fully verified (2026-08-22–23), including a real logged-in browser click-through; not yet CI-Verified.** D-AccountStatusEnforcement. `ResolveBySubject` (`internal/identity/adapters/store.go`) now filters `a.status = 'active'` alongside `deleted_at IS NULL`; a disabled account's identity lookup falls through to the existing `ErrIdentityNotFound` → uniform-401 path, no new error type. A second gap found during implementation and closed in the same pass: `LinkOnMatch`'s JIT re-link (`internal/identity/application/service.go`) resolved the person's account via `GetActiveAccountByPerson`, which never checked status either — a SQL-level fix there would have made `LinkOnMatch` mistake "found but disabled" for "no account" and auto-provision a *second*, active account for an already-disabled person. Fixed instead with an explicit `Account.Status` check in application code, returning the new `ErrAccountDisabled` sentinel. New `Deactivate`/`Reactivate`/`AccountStatus` on `internal/identity/application.Service` (idempotent; no self-lockout or last-admin guard, matching `RevokeInstanceAdmin`'s own existing precedent of having none). Exposed as `getAccountStatus`/`deactivateAccount`/`reactivateAccount` on `CoreSuperAdminService` (`api/core.conjure.yml`), inheriting the existing route-group `RequireInstanceAdmin` gate with no new wiring. UI: an "Account status" card on the person detail page (`web/apps/admin/app/[locale]/admin/(super-admin)/people/[personId]/page.tsx`), mirroring the instance-admin toggle's own form-action pattern; i18n added to all four locales. New integration test (`internal/identity/account_status_integration_test.go`, real Postgres) proves the disabled-account rejection on both the direct-resolve and JIT-relink paths, and that no duplicate account gets created. Live-verified twice against the dev stack: the full `TestAuthorizationMatrix` (extended with a `getAccountStatus` entry) passes with a temporary `DEV_ISSUER_HMAC_KEY` override, and a standalone HTTP round trip proved the actual behavioural effect end to end — an authenticated person's `whoami` call returns 200, then 401 immediately after an admin calls `deactivateAccount` on them, then 200 again after `reactivateAccount`, with `getAccountStatus` reads confirming `active`/`disabled` at each step. No migration needed, the column already existed. **Real browser click-through completed 2026-08-23** (a dedicated cross-milestone manual verification pass covering the whole M11 arc): deactivated a real test person as admin, confirmed their next real Google sign-in was rejected (NextAuth's own generic error page, not a crash — a minor messaging-polish gap noted but not fixed, since the security property itself held), then reactivated and confirmed sign-in succeeded again. |
| M11.2 · General admin audit log | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **Built (2026-08-22), not yet CI-Verified and not yet live-verified over real HTTP or a real browser session (unlike M11.1's own closing pass, no `DEV_ISSUER_HMAC_KEY` override or Google OAuth session was set up this session).** D-AuditLogShape. New append-only `identity_audit_log` table (`migrations/0016_core_audit.sql`), RID `new_id(1,1,4)` folded into the identity service (type 4, next free slot after person/account/external_identity — see D-AuditLogShape for why not a new service number), mirroring `moderation_actions`' proven append-only shape (`reject_mutation` trigger reused, not redefined; no `updated_at`/`deleted_at`). New self-contained `internal/auditlog` module (domain/application/adapters, no transport of its own — cursor logic lives in `internal/core/transport`, the same shape `internal/identity` already has) so M11.3's session revocation can reuse `Record` without reaching into `internal/core`. Wired into all six mutating `CoreSuperAdminService` paths: `GrantUnitRole`/`RevokeRoleAssignment`/`GrantInstanceAdmin`/`RevokeInstanceAdmin` (M10.8) and `DeactivateAccount`/`ReactivateAccount` (M11.1) — the exact set the ticket named, "no blind spot from day one." Two real gaps found and closed while wiring this in, not anticipated going in: `DeactivateAccount`/`ReactivateAccount` never resolved a caller subject from context at all (unlike their four siblings), and those four siblings themselves discarded `SubjectFromContext`'s `ok` bool rather than failing on a missing one — both now hard-fail via one shared `requireSubject` guard, closing the same class of gap M11.1's own discovery pass found once already, this time in all four affected call sites rather than three. `RevokeRoleAssignment`/`RevokeInstanceAdmin`'s store methods now `RETURNING` the revoked row's identity instead of an affected-rows count, and `InsertRoleAssignment` returns a real id including on its existing idempotent-conflict path, so every audit entry gets a meaningful before/after and `target_id` (see D-AuditLogShape for why curated per-call-site JSON over a generic row-diff). New `listAuditLog` endpoint on `CoreSuperAdminService` (`api/core.conjure.yml`), keyset-paginated and filterable by actor/target/date, cloning moderation's `listReports` cursor convention (own `Core:InvalidPageToken`, not shared cross-module, matching `congregationimport.conjure.yml`'s own precedent). UI: a new `/admin/audit-log` viewer reusing `web/apps/admin/components/data-table.tsx` — its `renderExpanded` prop's first real use anywhere in this app, showing before/after JSON on row-expand — cloned from `moderation/report-list.tsx`'s Load-more shape; a plain GET filter form (same convention `people/page.tsx`'s search box uses) rather than a Server Action, since the structured actor/target/date filters need a real server refetch through `listAuditLog`, not a client-side re-render over already-loaded rows; i18n added to all four locales; a new sidebar nav entry. Automated proof this session, all against real Postgres: `internal/auditlog/auditlog_integration_test.go` (keyset pagination correctness, actor/target/date filter predicates), `internal/core/core_super_admin_integration_test.go` (all six mutating paths asserted to write exactly one correctly-shaped audit row each, actor/target/before/after included, plus the missing-subject fail-loud path leaving no row and no side effect), `internal/core/transport/cursor_test.go` (cursor round-trip + the same tamper-case table moderation's own cursor test uses), and a new `listAuditLog` entry in `authorization_matrix_test.go` proving the route-group gate covers it too. `go build`/`go vet`/`go test ./...` and both `web/apps/{admin,web}`'s `tsc --noEmit`/`eslint` all clean; `docker compose up -d --build openfaithmap-api openfaithmap-admin` boots both clean with no panics in either container's logs, and an unauthenticated request to `/admin/audit-log` correctly redirects to `/login` (proving the route is wired into the existing `(super-admin)` layout gate) — but the actual live HTTP round trip with a minted dev token was not attempted this session. **Real browser click-through completed 2026-08-23** (cross-milestone manual verification pass): a real `UPDATE_PROFILE` audit row was confirmed rendering on `/admin/audit-log` with the correct actor, target, and before/after payload, immediately after a real profile edit through the UI. |
| M11.3 · Server-side session tracking + revocation | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **Built and live-verified over real HTTP (2026-08-22), not yet CI-Verified.** D-SessionTracking, plus a new D-SessionIdTransport recording two mechanics the original decision text left open (both confirmed with the user before implementation). New `migrations/0017_core_sessions.sql`: `identity_sessions` (RID `new_id(1,1,5)`, identity's next free object slot) — mutable, not append-only like `identity_audit_log`, since `last_seen_at`/`revoked_at` are updated post-insert. `internal/identity` gains `domain.Session`, `adapters.Store` session methods (`InsertSession`/`GetSession`/`ListActiveSessionsByAccount`/`TouchSession`/`RevokeSession`, the last throttled to a 60s-stale write), and `application.Service` methods (`RegisterSession`/`ListSessions`/`RevokeSession`/`ListMySessions`/`RevokeMySession`/`Touch`). **Session-id transport (D-SessionIdTransport):** since the API bearer is Google's own signed ID token — which can't carry a custom claim — the session id travels as its own opaque `X-Session-Id` header instead, stamped onto NextAuth's own cookie JWT at sign-in (`web/apps/admin/auth.ts`'s `jwt()` callback, which also now calls a new self-scoped `registerSession` endpoint over raw `fetch` to create the row) and sent on every API call via `lib/core.ts`'s `client()` (the existing `OpenFaithMapClientOptions.fetch` override hook). `internal/identity/middleware.Authenticator.Handle` reads it right after bearer resolution, cross-checking the returned account id against the bearer's own — a new `SessionChecker` interface plus `authz.Subject.SessionID`/`Issuer` fields (set once per request, reused by `RegisterSession` so the issuer is never client-supplied). One route, `CoreService.registerSession`, is exempt from needing a *pre-existing* session (`sessionExemptRoutes` — it's what creates the row), while still requiring a full bearer. **No issuer-based carve-out** (the user's explicit call): the reserved local/dev HS256 issuer is enforced identically, so `cmd/openfaithmap-api/authorization_matrix_test.go`'s `seedSubjects`/`doReq` and `scripts/mint-local-token` (new optional `-database-url`/`-account-id` flags, a deliberate opt-in break of its previous no-side-effects contract) both now mint a real session alongside every token. New `api/core.conjure.yml` types (`Session`/`SessionPage`/`RegisterSessionRequest`, `SessionNotFound` error) and endpoints: `listSessions`/`revokeSession` on `CoreSuperAdminService` (admin-scoped, mirroring `getAccountStatus`'s shape), `registerSession`/`listMySessions`/`revokeMySession` on `CoreService` (self-scoped, mirroring `whoami`). Every mutation audit-logged per M11.2 (`REVOKE_SESSION`/`SESSION`), `RegisterSession` deliberately not (a sign-in, not an admin action). UI: a "Sessions" card on the super-admin person detail page, mirroring the Account-status card's per-row form pattern; i18n added to all four locales (real translations, not English placeholders). The self-service "My sessions" panel is deliberately deferred to M11.5's own profile page, per this milestone's own original UI scope line — M11.3 ships the self-scoped backend endpoints only. Proof this session: `go build`/`go vet`/`go test ./...` clean, including a new `internal/identity/session_integration_test.go` (cross-account revoke rejection, idempotent revoke, `ErrSessionRevoked` after revoke) and an extended `core_super_admin_integration_test.go` (`RevokeSession`'s audit row); both `web/apps/{admin,web}`'s `tsc --noEmit`/`eslint` clean after the SDK regeneration. `docker compose up -d --build openfaithmap-api openfaithmap-admin` booted both clean with no panics; a full live HTTP round trip against the rebuilt stack (temporary `DEV_ISSUER_HMAC_KEY` override, cleaned up after) proved the mechanism is actually load-bearing, not just present in code — a fresh bearer 401s on `whoami` with no session, `registerSession` succeeds anyway (proving the exemption), the resulting session then lets `whoami` return 200, and `revokeMySession` on that same session makes the identical bearer 401 again. **Real browser click-through completed 2026-08-23** (cross-milestone manual verification pass): the Sessions card on a real signed-in `/whoami` page correctly showed device label, last-active timestamp, and a `(current)` marker on the active session. |
| M11.4 · Last-login / activity tracking | ✅ | ✅ | ✅ | ➖ | ✅ | ✅ | **Built and live-verified over real HTTP (2026-08-22), not yet CI-Verified.** Derives entirely from M11.3's `identity_sessions.last_seen_at`, no new migration, as the milestone's own text predicted. One mechanic left open at decision time was settled with the user before implementation: **last-active is revoked-inclusive** — `MAX(last_seen_at)` across an account's sessions whether or not they're revoked, matching `GetSession`'s own existing "revoked or not" read convention, so an admin revoking a session doesn't retroactively erase that the person was active. New `internal/identity/adapters.Store.LastActiveAtByAccount` (single-account, backs the detail page) and a join added directly to `SearchPersons`' own query (batched in one round trip, backs the people list) — both revoked-inclusive, neither touching `ListActiveSessionsByAccount`'s existing revoked-filtered query used elsewhere. Deliberately **not** wired into `GetPerson`/`GetPersons`/`scanPerson` (the non-admin `CoreService` reads `Person` also backs, e.g. my-congregation): those simply never populate the new field since their SQL doesn't join `identity_sessions`, so no session-derived signal reaches a non-admin caller. The field lands on the existing shared `Person`/`AccountStatus` conjure types rather than a new super-admin-only type, following the precedent `searchPersons`/`getAccountStatus` already set of reusing shared types for admin-gated endpoints. UI: a last-active line added to each people-list row and to the person detail page's existing "Account status" card (`web/apps/admin/app/[locale]/admin/(super-admin)/people/{page.tsx,[personId]/page.tsx}`); i18n added to all four locales. New `internal/core.TestLastActiveIntegration` (real Postgres, in `core_super_admin_integration_test.go`) — deliberately a separate test function from `TestSuperAdminAuditTrailIntegration`, since `GetAccountStatus`/`SearchPersons` are pure reads that never touch `identity_audit_log`, sidestepping that test's own cleanup (see below) entirely — proves no-account/no-session/has-session/revoked-session across both read paths. `go build`/`go vet` clean; `go test ./...` clean except a **pre-existing environment issue confirmed unrelated to this milestone** (reproduced identically against the pre-M11.4 commit): the shared dev Postgres's `openfaithmap` role isn't owner of `identity_audit_log`, so `TestAuditLogIntegration`/`TestSuperAdminAuditTrailIntegration`'s own cleanup (`ALTER TABLE ... DISABLE TRIGGER`) fails with `must be owner of table` — a role-permission gap in the shared dev instance, not a code defect, and not touched by this milestone. Both `web/apps/{admin,web}`'s `tsc --noEmit`/`eslint` clean. `docker compose up -d --build openfaithmap-api openfaithmap-admin` booted both clean with no panics. Live-verified over real HTTP (temporary `DEV_ISSUER_HMAC_KEY` override + `scripts/mint-local-token`, cleaned up after): a throwaway account's session was backdated two hours, and both `searchPersons` and `getAccountStatus` returned the matching `lastActiveAt` over the wire; the session was then revoked via a real `DELETE .../sessions/{id}` call, confirmed `revoked_at` was actually set in Postgres, and `getAccountStatus` still returned the same unchanged `lastActiveAt` — proving the revoked-inclusive decision end to end, not just at the unit level. **Real browser click-through completed 2026-08-23** (cross-milestone manual verification pass): the people-search list correctly showed a real last-active timestamp for a signed-in person's row. |
| M11.5 · Self-service profile page | ✅ | ✅ | ✅ | ➖ | ✅ | ✅ | **Built and live-verified over real HTTP (2026-08-22), not yet CI-Verified.** One scope question, open at decision time, was settled with the user before implementation: the "read-only role list" is a genuine new self-scoped `listMyRoleAssignments` read (not just `Whoami`'s existing `isInstanceAdmin` flag), the user's explicit reasoning being auditability of one's own grants plus a deliberate BOLA/IDOR posture — personId must be derived strictly server-side from the resolved session subject, never a request parameter. `internal/authz`: new `ListRoleAssignmentsByPerson` (store + service), a straight mirror of the existing `ListRoleAssignmentsByUnit` filtered on `subject_person_id` instead of `target_unit_id` — the first per-person (cross-unit) role read in the codebase. `internal/identity`: new `UpdateDisplayName` (`adapters/store.go`) — the first person-mutation store method in the module (`GetPerson`/`GetPersons`/`SearchPersons` are all reads; `InsertPerson` is boot/registration-only) — plus a thin `UpdateMyProfile` application wrapper. `internal/core/application/service.go` is where the BOLA defense actually lives: `UpdateMyProfile` and `ListMyRoleAssignments` both resolve the target person id exclusively from `requireSubject`/`authz.SubjectFromContext`, never a client-supplied argument; `UpdateMyProfile` is still audit-logged (`UPDATE_PROFILE`/`PERSON`) even though self-initiated, matching M11.2's every-mutation convention and `RevokeMySession`'s own precedent for that reasoning. New `api/core.conjure.yml` `CoreService` endpoints `updateMyProfile` (`PUT /profile`) and `listMyRoleAssignments` (`GET /profile/roles`, reusing the existing `RoleAssignmentPage` type), plus `UpdateMyProfileRequest` (deliberately no `personId` field, by the same reasoning `RegisterSessionRequest` already documents for `issuer`). UI: `whoami`'s raw-JSON dump (`web/apps/admin/app/[locale]/(chrome)/whoami/page.tsx`) is now a real profile page — an editable display-name form (mirrors the super-admin person-detail page's `toggleAccountStatus` server-action pattern), the role list, and the "My sessions" panel M11.3 built self-scoped backend endpoints for but never gave a UI (per-row revoke, same pattern as the super-admin page's admin-scoped session revoke). i18n added to all four locales, real translations not English placeholders. Proof this session: `go build`/`go vet`/`go test ./...` clean, including a new `internal/identity/profile_integration_test.go` (display-name update persists, a second person's row is untouched) and a new `internal/core/core_self_service_integration_test.go` (no-subject-context gate on both new methods; exactly one `UPDATE_PROFILE` audit row with correct actor/target/before/after; **the concrete IDOR-safety proof** — two persons each granted a different role assignment, `ListMyRoleAssignments` under each one's own subject returns only their own row, never the other's) — both hit the same pre-existing shared-dev-Postgres `identity_audit_log` ownership gap M11.4's own row documents for cleanup only, not a code defect, confirmed by diffing against the already-passing `TestSuperAdminAuditTrailIntegration`'s identical cleanup failure. Both `web/apps/{admin,web}`'s `tsc --noEmit`/`eslint` clean. `docker compose up -d --build openfaithmap-api openfaithmap-admin` booted both clean with no panics; a full live HTTP round trip against the rebuilt stack (temporary `docker-compose.override.yml` `DEV_ISSUER_HMAC_KEY`, two real accounts+sessions via `scripts/mint-local-token`, removed after) proved the mechanism end to end, not just present in code: a bearer with no `X-Session-Id` still 401s on `whoami`; `listMyRoleAssignments` returns exactly the caller's own single assignment for each of two different accounts, cross-checked to confirm neither ever saw the other's; `updateMyProfile` persisted a new display name with a bumped `updatedAt`, wrote the expected audit row (verified directly in Postgres), and left the other account's row untouched; an unauthenticated request to `/whoami` on the rebuilt admin app correctly redirects to `/login`. **Real browser click-through completed 2026-08-23** (cross-milestone manual verification pass): a real display-name edit through `/whoami`'s Profile card persisted correctly across a page reload. |
| M11.6 · Invite-a-teammate flow (link-based) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **Built and live-verified over real HTTP (2026-08-22), not yet CI-Verified.** D-InviteLinkMVP. Two design points the milestone text left open were settled with the user before implementation: **invite tracking** is a real new `identity_invites` table (`migrations/0018_core_invites.sql`, RID `new_id(1,1,6)`) rather than a stateless signed JWT — `status` is `pending`→`accepted` only, no stored `expired`/`revoked` value (expiry checked live against `expires_at`; revocation deliberately reuses M11.1's existing account deactivate/reactivate rather than a second status column — `ResolveInvite` also checks the linked account's own status, so deactivating the pre-provisioned account already invalidates a bad invite, live-verified). **The landing experience** is a dedicated unauthenticated `/accept-invite?token=...` page (`web/apps/admin/app/[locale]/(chrome)/accept-invite`), not a bare link to `/login`. Token shape: a random 32-byte value, base64url-encoded for the link; only its SHA-256 hash is ever persisted (`token_hash`) — deliberately not a signed HMAC/JWT, which would need a new production secret and still couldn't be truly one-time without a DB row anyway. `internal/identity`: `CreateInvite` (checks `GetActiveAccountByEmail` first, `ErrAccountAlreadyExists` before any insert) and `ResolveInvite`; a new `Store.InsertPersonAccountInvite` runs the Person+Account+Invite writes in ONE transaction — a real gap caught by testing, not anticipated going in: without it, a failure on the last insert left an orphaned active Person+Account with no Invite, permanently blocking any future invite to that email (the duplicate-email check would keep seeing it as taken). `LinkOnMatch` gains one call to `MarkInviteAcceptedByAccount` right after its existing `InsertIdentity` — the exact moment JIT actually links the account, not the moment the invitee views the landing page. **A second real gap found live, not in review:** `resolveInvite` cannot live on `CoreService` at all — Conjure's `default-auth: header` is a fixed per-service choice enforced by generated code itself (`httpserver.ParseBearerTokenHeader`, independent of `internal/identity/middleware`'s own bypass list), so an anonymous endpoint needs its own service. New `CorePublicService` (`api/core.conjure.yml`, base-path `/core/v1/public`, no `default-auth`) mirrors `ContentPublicService`'s exact shape; `isBypassPath` gained a matching `/core/v1/public` prefix bypass, the same mechanism `/content/v1/public` already uses. **A third gap, a boot-time panic caught live:** `POST /persons/invite` under `CoreSuperAdminService` collided with the existing `/persons/{personId}` wildcard route — httprouter's radix tree refuses a static segment as a sibling of a wildcard at the same depth — resolved by moving invite creation to a top-level `POST /invites` (it has no existing `personId` to path-parameter against anyway). UI: `/admin/people/invite` (a new client-component form using React 19's `useActionState` to receive the generated link directly from its Server Action instead of redirecting — every existing M11.1/M11.3 Server Action ends in `redirect()`, which would either discard the one-time token or leak it into the URL/browser history/server logs) plus the `/accept-invite` landing page; i18n added to all four locales, real translations. Proof this session: `go build`/`go vet`/`go test ./...` clean except the same pre-existing shared-dev-Postgres `identity_audit_log` ownership gap M11.4/M11.5 already document (cleanup-only, not a code defect) — including a new `internal/identity/invite_integration_test.go` (create/duplicate-email/resolve/expired/already-accepted/disabled-account/JIT-acceptance-hook, real Postgres) and an extended `TestSuperAdminAuditTrailIntegration` (`InvitePerson`'s audit row, `requireSubject` gate). Both `web/apps/{admin,web}`'s `tsc --noEmit`/`eslint` clean, `make sdk-verify` clean. `docker compose up -d --build openfaithmap-api openfaithmap-admin` booted both clean with no panics (after the two fixes above). `TestAuthorizationMatrix` extended with `invitePerson` (instance-admin gated, 409 on a deliberately-reused seeded email so repeated runs leave no new row) and a new `resolveInvite_anonymous_bypass` case proving the bypass is real. Full live HTTP round trip against the rebuilt stack (temporary `docker-compose.override.yml` `DEV_ISSUER_HMAC_KEY` + `IDENTITY_JIT_*`, a throwaway instance-admin account via direct SQL + `scripts/mint-local-token`, all removed/cleaned up after): `invitePerson` created a real Person+Account+Invite and returned a token; `resolveInvite` with **no bearer at all** returned the invitee's name/email; minting a bearer for the invitee's own never-before-seen (email, subject) and calling `registerSession` alone (JIT's first-sign-in path) linked the account, and a follow-up `whoami` returned 200 with the exact `personId`/`accountId` `invitePerson` had returned; the invite's `identity_invites.status` was confirmed `accepted` directly in Postgres, and a second `resolveInvite` on the same token correctly 409'd `Core:InviteAlreadyAccepted`; a second invite's account was deactivated via the existing M11.1 endpoint and its `resolveInvite` call then 404'd `Core:InviteNotFound` — the revocation-via-deactivation design proven end to end, not just at the unit level. **Real browser click-through completed 2026-08-23** (cross-milestone manual verification pass): the full invite-accept loop was completed with a second real Google account — invite created, link opened, Google sign-in completed, and JIT-linked correctly onto the invited person (confirmed via `/whoami` showing the right name/email/roles in that account's own session). This exact flow also surfaced a real, pre-existing bug in the admin app's shared session layer (`web/apps/admin/auth.ts`): a token whose `expiresAt` was never populated on initial sign-in was treated as "still valid forever" rather than "needs an immediate refresh," so it silently went stale and crashed every subsequent page load with an unhandled 401. Fixed by inverting that fallback condition — unrelated to this milestone's own scope, found only because this was the first real multi-account Google sign-in sequence this environment had exercised. |
| M11.7 · Bulk role assignment | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **Built and live-verified over real HTTP (2026-08-22), not yet CI-Verified.** A batch variant of the existing `GrantUnitRole` (`internal/authz`): `Store.BulkInsertRoleAssignments` grants roleId/unitId to every id in a `personIds` batch inside one real `pgx.Tx` (`internal/authz/adapters/store.go`) — no new authorization model, reuses the `CoreSuperAdminService` route-group's existing `RequireInstanceAdmin` gate with no new per-handler wiring. New `bulkGrantUnitRole` endpoint (`POST /bulk-role-assignments`, a fresh top-level resource deliberately not nested under `/role-assignments/` — M11.6's own `POST /persons/invite` boot-time radix-tree collision was the precedent this avoids), `core.application.Service.BulkGrantUnitRole` (requireSubject → the one atomic store call → a best-effort per-row `auditLog.Record` loop over the resulting assignment ids, reusing M11.2's convention: one `BULK_GRANT_UNIT_ROLE` audit row per grant, only ever reachable after the transaction has already committed). UI: a new client component (`role-grants/bulk-grant-form.tsx`) adds a people-search-with-checkboxes panel to the existing role-grants page (the unit already chosen there) — the app's first row-selection UI, plain `<input type="checkbox">`, no new dependency; the existing single-person grant form is kept alongside it, not replaced. Real es/pt/uk translations added for the new strings, plus the pre-existing `roleLabel`/`rolePlaceholder` gap (referenced by the page since M10.8 but never defined in any locale file) fixed in the same pass. **A real, live pre-existing bug found by this milestone's own integration test, not anticipated by any prior plan**: `authz_role_assignments_active_idx` (`migrations/0009_core_authz.sql`) has no `NULLS NOT DISTINCT`, and `graph_id` is always NULL for scope `'unit'` — Postgres treats NULL as distinct from NULL for uniqueness by default, so this index was silently never enforcing uniqueness for the vast majority of grants in this app, and `InsertRoleAssignment`'s own claimed idempotent-conflict fallback (catching a `23505` on this index) was dead code the whole time; repeated identical unit-scope grants have always been silently accumulating as duplicate active rows rather than being deduplicated (no PDP-decision impact — duplicate rows for the same person/role/unit grant identical permissions — but a real, visible defect on the role-grants screen, each duplicate rendering as its own separately-revocable row). Fixed at the root with a new migration, `migrations/0019_core_role_assignments_nulls_not_distinct.sql` (`DROP`/`CREATE UNIQUE INDEX ... NULLS NOT DISTINCT`, confirmed zero pre-existing duplicate rows before applying), which is also what makes `BulkInsertRoleAssignments`'s own `ON CONFLICT ... DO UPDATE` upsert (deliberately not a per-row catch-then-select loop like the single-grant path — sharing one explicit `pgx.Tx` across a batch means a caught `23505` would abort the whole transaction, SQLSTATE `25P02`, failing every subsequent statement including the rest of the batch) actually function as designed. Proof this session: `go build`/`go vet`/`go test ./...` clean; new `internal/core/core_super_admin_integration_test.go`'s `TestBulkGrantUnitRoleIntegration` (real Postgres) proves the happy path (3 grants, 3 audit rows), the real rollback (`ErrPermissionDenied` and FK-violation cases each leave zero rows, not a partial apply), and the specific in-batch idempotent-conflict case the `NULLS NOT DISTINCT` fix exists to make possible (a pre-existing grant re-submitted inside a batch upserts instead of duplicating) — this last case is the one that caught the pre-existing bug live, failing loudly before the migration was added. `TestAuthorizationMatrix` extended with a `bulkGrantUnitRole` entry (empty `personIds` → `400 Core:EmptyPersonIdsList`, side-effect-free, proving the same route-group gate covers it). Both `web/apps/{admin,web}`'s `tsc --noEmit`/`eslint` clean, `make sdk`/`make sdk-verify` clean. `docker compose up -d --build openfaithmap-api openfaithmap-admin` booted both clean with no panics — including the deliberate route-collision smoke check the new endpoint's path was chosen to avoid — and the new migration applied cleanly through the real `openfaithmap-migrate` Atlas service. Full live HTTP round trip against the rebuilt stack (temporary `docker-compose.override.yml` `DEV_ISSUER_HMAC_KEY`/`IDENTITY_JIT_*`, a throwaway instance-admin via direct SQL + `scripts/mint-local-token`, all removed/cleaned up after): a real 3-person batch grant returned `204`, all 3 assignments and exactly 3 correctly-shaped `BULK_GRANT_UNIT_ROLE` audit rows confirmed directly in Postgres; a second batch with one syntactically-valid-but-nonexistent person id `400`'d and left zero new rows for the batch's other, real person — the rollback proof repeated over real HTTP, not just at the integration-test level; the full `TestAuthorizationMatrix` suite passed against the live stack, `bulkGrantUnitRole` included. **Real browser click-through completed 2026-08-23** (cross-milestone manual verification pass): a real bulk grant through `/admin/role-grants` succeeded, with the resulting assignment and its `BULK_GRANT_UNIT_ROLE` audit row both confirmed in the UI (one hiccup along the way, not a bug: the unit ID initially handed to the tester was stale `.env` data from a prior Postgres volume, causing a real `authz_role_assignments_target_unit_fk` violation until a genuinely current unit ID from the live database was used instead). |
| M11.8 · Person merge/dedupe tooling | ✅ | ✅ | ✅ | ➖ | ✅ | ✅ | **Built and fully verified (2026-08-22–23), including a real logged-in browser click-through; not yet CI-Verified.** The riskiest of the nine — touches every table keyed by `person_id`. `MergePersons(survivorID, duplicateID)` (`internal/core/application/service.go`) opens one transaction binding fresh tx-scoped stores from `identity`, `authz`, and `membership` — the same cross-module-transaction shape `internal/identity/bootstrap.Run` already established for the boot-time admin seed, just triggered at runtime — and calls each module's own new store method (`authz/adapters.RepointRoleAssignments`/`RepointInstanceAdmin`, `membership/adapters.RepointMemberships`, `identity/adapters.MergePersonIdentity`): every active row on the duplicate either repoints onto the survivor or, if the survivor already holds an identical one, gets revoked/ended as redundant (not duplicated) — the same collision shape M11.7's `BulkInsertRoleAssignments` faced, resolved here with `UPDATE ... WHERE NOT EXISTS (...)` instead of `ON CONFLICT`, since the input is "every row belonging to one person," not "N distinct new rows." **External identity conflict, decided with the user: soft-merge only** — re-pointing the duplicate's identity onto the survivor's own account (so the same human could keep logging in with either) was considered and declined; instead, when both survivor and duplicate already have their own active account, the duplicate's account is disabled (`status='disabled'`, `deleted_at=now()`) and its sessions revoked, its external identity left attached and unusable, never re-pointed. When the survivor has no account of their own, the duplicate's whole account simply moves onto the survivor (no conflict). **Scope, decided with the user: `registration`/`moderation`/`vouching`/`congregationimport` rows are explicitly out of scope** — they reference person ids as opaque `text` with no FK to `identity_persons`, matching this row's own original enumerated scope (role assignments, membership, sessions, external identities only); documented as a known limitation in `mergePersons`'s own Conjure `docs:` field, not a silent gap. No migration needed — every operation uses columns that already existed (`deleted_at`, `status`, `revoked_at`/`revoked_by`, `effective_to`); `identity_persons.status` (unused since M10.1) gets its first real write here. Two new `CoreSuperAdminService` endpoints, `previewMergePersons`/`mergePersons` (`POST /persons/{personId}/merge-preview` and `/merge`, depth-3 suffixes under the existing `/persons/{personId}` wildcard — confirmed safe by the four suffix routes already coexisting there, unlike M11.6's depth-2 static-sibling panic). UI: a "Merge with another person" card on the person detail page linking to a new `.../people/[personId]/merge` route — a plain-GET id field (matching `audit-log/page.tsx`'s own filter-form convention) then a preview Card with a `variant="destructive"` confirm button (this app's existing risk-communication convention; no confirm dialog exists anywhere in this app). New `internal/core/core_super_admin_integration_test.go`'s `TestMergePersonsIntegration` (real Postgres) proves the happy path, all three independent collision cases, the Case B account-conflict soft-merge, and the permission-denied/self-merge/not-found guards — green. Live-verified over real HTTP: `docker compose up -d --build openfaithmap-api openfaithmap-admin` rebuilt and booted both containers cleanly (proving the two new routes didn't trigger an httprouter radix-tree panic at boot, the exact M11.6 risk class), and both new endpoints answer a real `Identity:Unauthorized` (not a 404) when called unauthenticated from inside the compose network. **Real browser click-through completed 2026-08-23** (cross-milestone manual verification pass): merged a real duplicate test person into a survivor through `/admin/people/{id}/merge`'s preview → confirm flow; independently confirmed in Postgres afterward — the duplicate's `identity_persons.status`/`deleted_at` and `identity_accounts.status`/`deleted_at` all flipped as expected (soft-merge, both had their own active account), the survivor's own row was untouched, and exactly one `MERGE_PERSONS` audit row was written. Deliberately sequenced last among the higher-risk items, after sessions and the audit log already exist to make it observable/reversible. |
| M11.9 · API keys for programmatic access | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **Built and fully verified (2026-08-22–23), including a real logged-in browser click-through; not yet CI-Verified.** The last M11.x, and the arc's second new-authentication-mechanism milestone after OIDC itself. Two design forks the original spec left open were resolved with the user before building, not guessed: **permission scoping is a fixed allowlist chosen at key-creation time** (`identity_api_keys.permission_codes text[]`), not blanket inheritance of the owner's permissions — at request time the effective permission set is that allowlist intersected with the owner's LIVE `authz` grants, implemented as two independent gate checks in `internal/authz.Service.Require` (allowlist membership, then the completely unmodified `enforce`/`decide`/`ActiveGrantsForSubject` path) rather than a materialized set or a new `authz` migration — `authz_role_assignments.subject_person_id`'s hard `NOT NULL` FK to `identity_persons` is untouched, an API key stays a second credential for an existing person, never a new principal type (`authz.Subject`'s own "no machine-subject arm" doc comment, D-DirectTokenVerification, still holds). `RequireInstanceAdmin` denies every API-key-authenticated subject unconditionally, even when the owner genuinely is an instance admin — the instance-admin plane is the PDP's "allow everything" bypass, and letting a key ride it would make its allowlist meaningless the moment the owner is later promoted, with no re-issuance and no visible change to the key; `CreateApiKey` correspondingly rejects instance-scope codes at creation since they could never be exercised anyway. **Management is self-service + admin oversight** — every person creates/lists/revokes their own keys (`CoreService`, self-scoped, mirroring `listMySessions`/`revokeMySession`), and instance admins additionally get a metadata-only view/revoke surface over anyone's keys (`CoreSuperAdminService`, `GET`/`DELETE /persons/{personId}/api-keys[/{apiKeyId}]`, nested inside the existing `{personId}` wildcard the same safe way `/sessions` already is — not a new top-level resource, unlike M11.6/M11.7's collisions) for incident response — killing a compromised key without waiting on the owner. `gencore.ApiKey` carries no token/hash field at all, so the admin surface's "metadata only" guarantee is structural, not just convention; only the one-time `CreateApiKeyResult.token` ever carries the raw secret (`ofm_`-prefixed, 32 random bytes base64url-encoded, SHA-256 hash persisted — the same shape `newInviteToken` already established, prefix added so the authenticator can detect an API-key-shaped bearer with no DB round-trip or JWT-parse attempt). New resolution path in `internal/identity/middleware.Authenticator.Handle`: branches on the `ofm_` prefix immediately after extracting the raw bearer, before any JWT validation, to `ResolveByAPIKey` — and is unconditionally exempt from M11.3's `X-Session-Id` check by construction (never reaches that code path at all), a deliberate, documented exception to that check's "no issuer-based carve-out" rule since an API key has no NextAuth session to forward by definition. Migration `0020_core_api_keys.sql` (no `authz` schema change). New `internal/core/core_self_service_integration_test.go`'s `TestSelfServiceApiKeysIntegration` (real Postgres) proves the no-subject/zero-audit-rows guard, the IDOR-safety shape (person B can't see or revoke A's key), the audit trail never carries the secret, and the allowlist ∩ live-grants intersection against real `ActiveGrantsForSubject` SQL; `core_super_admin_integration_test.go`'s `TestApiKeysAdminOversightIntegration` proves the admin-revoke path uses a distinct `REVOKE_API_KEY_ADMIN` audit action (actor = the admin, not the owner) and that the wire response never carries a secret at the JSON level. `internal/authz/service_test.go` gained six fake-store unit tests covering the same intersection logic in isolation. UI: a new self-service `/api-keys` page (`web/apps/admin/app/[locale]/(chrome)/api-keys/`, linked from `whoami`'s nav) with the exact "shown once" `useActionState` pattern `people/invite/invite-form.tsx` established, plus a permission-code checkbox picker (`role-grants/bulk-grant-form.tsx`'s `Set<string>` toggle-state pattern — no prior permission-picker component existed) backed by a new self-scoped `listPermissionCatalog` endpoint (`internal/authz/domain.UnitScopedPermissionCodes`, excluding instance-scope codes); a new "API keys" card on the super-admin person-detail page for the admin-oversight surface. i18n added to all four locales. Live-verified over real HTTP: `docker compose build && up -d openfaithmap-api` rebuilt and booted the container cleanly (no httprouter panic — the exact M11.6/M11.7 risk class, and the self-service `/api-keys` top-level routes plus the admin `/persons/{personId}/api-keys` nested routes were both new path shapes worth checking), and `GET /core/v1/api-keys`, `GET /core/v1/permission-catalog`, and `GET /core/v1/super-admin/persons/x/api-keys` all answered a real `Identity:Unauthorized` (not a 404) when called unauthenticated from inside the compose network. **Real browser click-through completed 2026-08-23** (cross-milestone manual verification pass): created a real key through `/api-keys`, confirmed the raw `ofm_...` secret rendered once with a copy button; used that exact secret as a bearer directly against the live API over the docker network with no `X-Session-Id` header — `GET /core/v1/whoami` returned 200 (and, notably, `isInstanceAdmin: false` even though the owning account genuinely is an instance admin, since `Whoami()` internally calls the now API-key-aware `RequireInstanceAdmin`), while `GET /core/v1/super-admin/roles` correctly 403'd; revoked the key both via self-service and, for a second test person's key, via the admin-oversight card, with both revocations and their distinct `REVOKE_API_KEY`/`REVOKE_API_KEY_ADMIN` audit rows confirmed directly in Postgres. This pass also found and fixed a real, pre-existing bug unrelated to the backend work: `web/apps/admin/proxy.ts`'s locale-middleware matcher excluded any path merely *starting with* the letters `api` (meant to exclude Next.js's own `/api/*` route handlers like NextAuth's callback) rather than requiring a trailing slash, so the new `/api-keys` page was silently excluded from locale-prefixing and 404'd on a bare request — the same "new route collides with an existing broad match" class of bug M11.6/M11.7 already found on the backend's own router, just surfacing here on the frontend's. Fixed by requiring `api/` (with the slash) in the exclusion. |
| M12 · Unit management | ✅ | ✅ | ➖ | ➖ | ➖ | ✅ | **Decided (2026-08-24).** A discovery pass (codebase audit plus external research against comparable ChMS products — Planning Center, Breeze, CCB, MinistryPlatform, diocesan tools — and mature enterprise org-hierarchy/RBAC platforms — AWS Organizations, Entra Administrative Units, Google Workspace OU-scoped roles, Okta, GitHub Teams) found this app's unit hierarchy (multi-graph model, materialized closure, unit/subtree-scoped RBAC, PDP decision tracing) already outclasses most of what was researched — the real gap is that units are **read-only by design**: `api/core.conjure.yml` exposes only reads plus role-assignment writes; the only unit-creating endpoint (`createChildOrg`) is narrow and purpose-specific; no update/delete/archive/suspend endpoint exists despite `Unit.State` already modeling it; reparenting only exists coupled inside `internal/registration`'s approval flow; `internal/directory` (the module that actually owns `CreateUnit`/`AddEdge`/`RemoveEdge`) has no HTTP transport at all; and `unit.lifecycle`/`unit.edges.manage` are seeded on real roles but never enforced anywhere. Scoped jointly with the user into eight sub-milestones (M12.0–M12.7, decided but not yet built) plus two new decisions in `architecture/decisions.md`: D-UnitMoveDualScope, D-TopDownUnitAccessOnly. Self-service join/request-to-join flows, full PIM-style eligible/active role states with approval workflows, reintroducing multi-tenancy, and a canonical fixed-depth hierarchy were all considered and explicitly left out. See the `M12 · Unit management` detail section below for the full scope narrative. **Verified (2026-08-26)** — see that section's own closing note for the two real bugs a browser click-through found and this pass fixed (an M11.3 session-id regression across six admin `lib/*.ts` files, and a same-parent `moveUnit` data-corruption bug) before flipping every sub-row below. |
| M12.0 · Scope down root-subtree grants | ✅ | ✅ | ✅ | ✅ | ➖ | ✅ | **Built and live-verified over real HTTP (2026-08-24), not yet CI-Verified.** The audit (`U14`) found the milestone's own framing didn't match the code on two points, corrected in a `D-PlatformModerator` addendum (`docs/architecture/decisions.md`): (1) every real/test-covered grant is `scope='unit'`, never `subtree` — `internal/authz/adapters/store.go`'s `InsertRoleAssignment`/`BulkInsertRoleAssignments` hardcode `'unit'`, and the wire types carry no scope field at all, so `subtree` grants are unprovisionable through any surface today (left as a real, open item for M12.2, which needs real reach to non-root parent units for D-UnitMoveDualScope's dual-parent check to mean anything); (2) `unit.lifecycle` was not actually dead — `internal/moderation`/`internal/vouching` `Require` it today as platform-moderator's own identity marker, unrelated to real unit-lifecycle mutation. Left unsplit, M12.1's real `setUnitState`/`deleteUnit` would have silently handed every platform-moderator archive/suspend/delete power over every unit under root. **Decided with the user**: split it into a new dedicated `PermModerationStanding` (`moderation.standing`, `migrations/0021_core_moderation_standing_permission.sql` — a `DELETE`+`INSERT` on platform-moderator's `authz_role_permissions` row, no assignment-table change), repointing `internal/moderation`/`internal/vouching`'s `authorize.go`; `unit.lifecycle` itself is untouched and free for M12.1. **`registration-operator`'s `unit.edges.manage` is deliberately left untouched and deferred to M12.2** — per `D-EdgePerms` it was seeded specifically to back a future generic move, and root-scope actually matches that job; the real open question (given finding 1 above) is how it would ever reach a non-root parent at all, which only makes sense to answer once M12.2 builds the real call site. Proof: `go build`/`go vet`/`go test ./...` clean; the new migration applied cleanly through the real `openfaithmap-migrate` Atlas service and confirmed directly in Postgres (`authz_role_permissions` shows `moderation.standing`, not `unit.lifecycle`, for platform-moderator); `TestAuthorizationMatrix`'s renamed `root_scoped_moderation_standing` case re-run against a rebuilt `openfaithmap-api` container over the real compose network — `platform-moderator` still `200`s on `/moderation/v1/reports`/`/vouching/v1/vouches`, everyone else still `403`s — proving the repoint preserved every existing allow/deny outcome exactly. |
| M12.1 · Unit lifecycle CRUD (create/update/state/delete) | ✅ | ✅ | ✅ | ➖ | ➖ | ✅ | **Verified (2026-08-26): CI green on PR #28 (`build-test`/`godel verify`/`sdk-verify`/both `web` legs), merged to `main`.** Built and live-verified over real HTTP (2026-08-24). No migration needed — `directory_units` already had `deleted_at`/`state` from `migrations/0010_core_directory.sql`, unused by app code until now. Added to `internal/directory`: `UpdateUnit`, `SetUnitState`, `HasChildren`, `DeleteUnit` (soft-delete via `deleted_at`; existing `ListAncestors`/`ListDescendants` already join `deleted_at IS NULL`, so a deleted unit vanishes from every hierarchy read with no edge/closure cleanup needed). Added to `internal/core` (composed per `D-InProcessAuthz`): `CreateUnit` (general creation under a parent, without `createChildOrg`'s religion-profile side effects), `UpdateUnit`, `SetUnitState`, `DeleteUnit` — all gated on `unit.lifecycle` over the target unit (`unit.edges.manage` stays reserved for M12.2 per `D-EdgePerms`), root unit hard-guarded against `SetUnitState`/`DeleteUnit` (`ErrRootUnitProtected`), `DeleteUnit` orphan-checked in cheapest-first order (children, then org profile, then active role assignments — reordered from the original children-last design once a real Postgres test proved the order matters when more than one condition holds at once), every mutation audit-logged (`CREATE_UNIT`/`UPDATE_UNIT`/`SET_UNIT_STATE`/`DELETE_UNIT`, `UNIT` target — this arc's first unit-targeted audit rows). New `api/core.conjure.yml` endpoints: `createUnit` (`POST /units`), `deleteUnit` (`DELETE /units/{unitId}`) — both safe alongside the existing `POST /units/children` (`createChildOrg`) and `GET /units/{unitId}` family; `updateUnit`/`setUnitState` deliberately live under a new `/unit-lifecycle/{unitId}` resource, not `/units/{unitId}`, discovered live: the vendored httprouter v1.3.0 panics at server boot on any `POST /units/{unitId}/*` route because it cannot register a wildcard sibling next to the existing static `/units/children` at the same tree depth (a real ambiguity — a `POST /units/children` request could match either route) — `createChildOrg`'s path is the one already live in the admin UI (`web/apps/admin/lib/jurisdiction.ts`), so the new routes moved instead, following `grantUnitRole`'s own precedent of a sibling top-level resource rather than nesting under `/units/`. Proof: `go build`/`go vet`/`golangci-lint`/`go test ./...` clean; new `internal/directory` and `internal/core` integration tests against real Postgres (`TestUnitLifecycleIntegration` in both packages) cover the full CRUD surface, the gate denying an ungranted real (non-instance-admin) `unit.lifecycle` grant succeeding, and all three orphan-protection checks blocking then clearing; `TestAuthorizationMatrix`'s new `target_scoped_unit_lifecycle` case re-run against a rebuilt `openfaithmap-api` container over the real compose network — anonymous `401`s, a `religionorg.manage`-only caller `403`s, an instance-admin's create/update/state/delete round-trip all succeed, and `setUnitState` on the real root unit `409`s `Core:RootUnitProtected` — proving the gate, the root guard, and the new routes all work over real HTTP, not just in-process. |
| M12.2 · Generic move/reparent capability | ✅ | ✅ | ✅ | ✅ | ➖ | ✅ | **Verified (2026-08-26), but only after a real bug found in browser verification was fixed first — see the end of this row.** Built and integration-tested against real Postgres (2026-08-24). Extracted the add-before-remove, resumable, closure-safe move logic formerly private to `internal/registration.Service.Reparent`/`runReparentSteps` into `internal/directory` as `Move`/`GetMoveStatus`/`CurrentParent`, backed by a new `directory_unit_move_jobs` table (`migrations/0022_directory_unit_move_jobs.sql`) — `jurisdiction_reparenting_jobs` (`migrations/0001_registration.sql`) is left in place, untouched, as a frozen historical log per this repo's expand-only convention. `internal/registration.Service.Reparent`/`GetReparentStatus` are now thin callers over `directory.Move`/`GetMoveStatus`, mapping `MoveJob` onto the unchanged public `ReparentingJob` wire shape; `requireOperator`'s existing root-scoped `religionorg.manage` gate is untouched — D-UnitMoveDualScope's new dual-parent check applies only to the new generic `internal/core.MoveUnit`, not retroactively to registration's own approval-flow reparent (D-InProcessAuthz: `internal/directory` does no authorization itself). One correction found while generalizing: the ported unique-live-job index (`WHERE status <> 'FAILED'`) let a terminal VERIFIED row occupy the "live" slot forever, permanently blocking any later move of that unit — harmless for a single-use congregation reparent, a real bug for a repeatable generic capability, so the index (and `GetLiveMoveJob`'s query) exclude VERIFIED too (`directory_unit_move_jobs_live_idx`). **U14 resolved**: chose to build real `scope="subtree"` grant provisioning rather than design around `scope="unit"`-only reach — added `scope`/`graphId` to `GrantUnitRoleRequest`/`BulkGrantUnitRoleRequest` (`api/core.conjure.yml`), threaded through `internal/authz`'s `InsertRoleAssignment`/`BulkInsertRoleAssignments` (previously hardcoded `'unit'`) up through `internal/core.GrantUnitRole`/`BulkGrantUnitRole`, with `internal/core`-level validation (`ErrInvalidGrantScope`/`ErrSubtreeGrantRequiresGraph`/`ErrUnitGrantMustNotSpecifyGraph`) mirroring `authz_role_assignments_graph_scope`'s own CHECK; no PDP or schema change needed, since `subtree`-scope closure cascade was already fully implemented and merely unprovisionable. New `internal/core.MoveUnit`/`GetUnitMoveStatus`, gated per D-UnitMoveDualScope on `unit.edges.manage` over BOTH the unit's current parent (resolved via `CurrentParent`) and the requested new parent, root-guarded (`ErrRootUnitProtected`, same hard guard M12.1's `SetUnitState`/`DeleteUnit` use), audit-logged (`MOVE_UNIT`, target `UNIT`). New `api/core.conjure.yml` endpoints under a sibling `/unit-moves/{unitId}` resource (`moveUnit`/`getUnitMoveStatus`), avoiding M12.1's own httprouter wildcard-collision class. Proof: `go build`/`go vet`/`golangci-lint`/`go test ./...` clean; new `internal/directory` (`TestUnitMoveIntegration`) and `internal/core` (`TestMoveUnitIntegration`) integration tests against real Postgres cover the full happy path, the terminal-VERIFIED live-slot fix (a second successful move of the same unit to a third parent), cycle rejection (recorded as a FAILED job, not a Go error — same shape the former `Reparent` already had), a missing grant denying, a one-sided `scope="unit"` grant (old parent only) still denying, and — the direct proof the U14 fix works — a `scope="subtree"` grant on a jurisdiction ancestor reaching both a descendant unit's old AND new parent via the closure table, letting the move succeed; `internal/registration`'s own `TestRegistrationIntegration` (unchanged assertions) continues passing against the refactored `Reparent`, proving the caller-side migration preserved behavior. The rebuilt `openfaithmap-api` container boots clean over the real compose network with no httprouter panic, and the three new/changed endpoints (`moveUnit`, `getUnitMoveStatus`, `grantUnitRole`'s new scope param) each correctly `401` an anonymous caller — routing-level proof only; full authenticated real-HTTP verification (the bar M12.0/M12.1 met, via `DEV_ISSUER_HMAC_KEY` + `TestAuthorizationMatrix`) was not done this session since no `docker-compose.override.yml`/dev-issuer key exists in this environment — left as the remaining gap before flipping Verified. **Verified (2026-08-26), after fixing a real same-parent-move data-corruption bug a browser click-through found** — see this milestone's own detail section below for the full account; CI green on PR #28, merged to `main`. |
| M12.3 · Role-assignment expiry (write path + display fix) | ✅ | ✅ | ✅ | ➖ | ✅ | ✅ | **Built and live-verified (2026-08-25), not yet CI-Verified.** Independent of M12.0–M12.2. No migration (`expires_at` has existed since M10.1) and no new permission code, confirming the design's own framing. Threaded `expires_at` through `InsertRoleAssignment`/`UpsertRoleAssignment` (`internal/authz/adapters/queries/authz.sql`, `repository.go`, `service.go`) and added it to `ListRoleAssignmentsByUnit`/`ListRoleAssignmentsByPerson`'s SELECT and `domain.RoleAssignment`; `internal/core.GrantUnitRole`/`BulkGrantUnitRole` gained an `expiresAt` param, validated in the future (`ErrExpiryInPast`, new `ExpiryInPast` conjure error) before ever reaching the store. New `internal/authz.ClearRoleAssignmentExpiry`/`internal/core.ClearRoleAssignmentExpiry`, mirroring `RevokeRoleAssignment`'s own shape exactly, audit-logged as `CLEAR_ROLE_ASSIGNMENT_EXPIRY`. New `api/core.conjure.yml` endpoint `clearRoleAssignmentExpiry` (`POST /role-assignments/{assignmentId}/clear-expiry`) — a deeper path segment under `revokeRoleAssignment`'s own `{assignmentId}` node, not a new top-level resource, avoiding M12.1/M12.2's own httprouter-collision class. Admin UI (`role-grants/page.tsx`, `bulk-grant-form.tsx`): a datetime-local expiry field on both the single- and bulk-grant forms, an "Expired" `StatusBadge` plus a "Clear expiry" action on each row once `expiresAt` is returned, and the same expired badge on `whoami/page.tsx`'s self-service role list (read-only there). `make sdk` regenerated the TypeScript client — which, incidentally, brought the admin app's stale-since-M12.2 generated types (`scope`/`graphId` had never actually been wired into any UI form) back in sync; `grantUnitRole`/`bulkGrantUnitRole`'s `lib/core.ts` wrappers now explicitly hardcode `scope: "unit"` to preserve every existing grant's prior behavior, since no scope-picker UI exists yet (a real, separate gap, not this milestone's to fix). Proof: `go build`/`go vet`/`go test ./...` clean; extended `internal/authz`'s `TestAuthzAdminSurfaceIntegration` and `internal/core`'s `TestSuperAdminAuditTrailIntegration` against real Postgres — grant-with-expiry round-trips through both list queries, a directly-expired row is correctly denied by `DecideFor` (the PDP's own `ActiveGrantsForSubject` filter, now exercised with a real non-NULL value for the first time), `ClearRoleAssignmentExpiry` clears it and audit-logs it, and a past `expiresAt` is rejected with `ErrExpiryInPast` before any store call — all assertions pass (the latter test's `t.Cleanup` still hits this local environment's pre-existing `identity_audit_log` ownership gap, unrelated to this milestone and identical on the base commit). The rebuilt `openfaithmap-api` container boots clean with no httprouter panic, and the new `clear-expiry` route correctly `401`s an anonymous caller over the real compose network — routing-level proof only, the same bar M12.2 accepted, since this environment still has no `docker-compose.override.yml`/dev-issuer key for full `TestAuthorizationMatrix`-style authenticated HTTP coverage. Admin app: `next build` (both `openfaithmap-admin` and `openfaithmap-web`) and `tsc --noEmit` clean; the rebuilt `openfaithmap-admin` container boots and correctly redirects an unauthenticated `/admin/role-grants` request to `/login` (200) — a real browser click-through of the grant/expire/clear-expiry flow itself was not done this session, since this app has no dev-login bypass (Google OAuth only) and no real Google account was available in this environment; left for the user's own browser pass against the now-running `localhost:3004`. |
| M12.4 · PDP `Explain`/`Via` exposure | ✅ | ✅ | ✅ | ➖ | ✅ | ✅ | **Built and live-verified (2026-08-25), not yet CI-Verified.** Independent of every other M12.x. `internal/authz/domain/pdp.go`'s `Decision`/`Contribution`/`Via`/`DenyReason` explain-tracing engine needed zero changes — it already fully implemented `Explain=true`, covered by `TestPDPDecideExplain`, just never called from an exported `Service` method. Added `authz.Service.ExplainDecision` (calls the already-private `decide(..., explain=true)`) and `core.application.Service.ExplainAccess` (validates the permission code against the closed catalog, then composes into `ExplainDecision` — no `requireSubject`/audit-log call, matching `ListRoles`/`SearchPersons`/`GetUnitMoveStatus`'s own "pure read, route-group gate is enough" shape). New `explainAccess` endpoint (`GET /access-decisions/explain`, three required query params) on `CoreSuperAdminService`, a brand-new top-level static resource with no `{}` segments — every existing depth-1 segment under this base path was already static, so this adds no httprouter wildcard-collision risk. **A real, small cleanup along the way**: `identitydomain.ErrUnknownPermissionCode` (M11.9's `CreateApiKey` allowlist check) duplicated a concept `authzdomain` already owned the check for (`IsKnownPermission`); relocated the sentinel into `authzdomain`, repointed `CreateApiKey`'s two call sites, and `core/transport/errors.go`'s `mapErr` keeps exactly one case instead of two spellings of the same error. Also fixed unrelated pre-existing `gofmt`/lint drift left by the earlier sqlc-conversion commits (three stale-formatted files, one dead helper in `internal/religion/adapters`) that was silently failing `godelw verify` before this session — a separate `chore:` commit, not folded into M12.3/M12.4. Admin UI: new `/admin/explain-access` page under `(super-admin)` (audit-log's plain-GET-form template, not role-grants' mutation-form one) with three plain-text-id inputs, an Allow/Deny `StatusBadge`, and a `Via`-contribution list or deny-reason line; a new sidebar entry under `SUPER_ADMIN_NAV`. Proof: `go build`/`go vet`/`golangci-lint`(`godelw verify`)/`go test ./...` clean; new `internal/authz` (`TestAuthzAdminSurfaceIntegration`, extended) and `internal/core` (`TestExplainAccessIntegration`, new) integration tests against real Postgres cover an allow with a matching `Via` contribution, a deny with a non-empty `DenyReason`, an unknown-permission-code rejection, and — the one behavior this milestone's own scope calls out — an explicit assertion that `identity_audit_log`'s row count doesn't grow across three `ExplainAccess` calls; `make sdk`/`sdk-verify` clean; both `openfaithmap-admin` and `openfaithmap-web` `next build`+`eslint` clean. The rebuilt `openfaithmap-api` container boots clean with no httprouter panic, and the new `access-decisions/explain` route correctly `401`s an anonymous caller over the real compose network; the rebuilt `openfaithmap-admin` container correctly `307`s an unauthenticated `/admin/explain-access` request to `/login` — routing-level proof only, the same bar M12.2/M12.3 accepted, since this environment still has no `docker-compose.override.yml`/dev-issuer key for full `TestAuthorizationMatrix`-style authenticated HTTP coverage (a new case was added to that matrix for whenever that key exists). A real browser click-through of the explain-access form itself was not done this session, for the same no-dev-login-bypass reason M12.3 documented; left for the user's own browser pass against the now-running `localhost:3004`. **Migrations squashed again, same session (2026-08-25), same precedent as M10.9's 23→15 pass**: 23 files → 20. `migrations/0019_core_role_assignments_nulls_not_distinct.sql` (M11.7's `NULLS NOT DISTINCT` fix), `0021_core_moderation_standing_permission.sql` (M12.0's permission split), and `0023_directory_unit_move_jobs_parent_idx.sql` (M12.2's missing indexes) — each a pure patch on an earlier file, no structural content of its own — are folded directly into what they patch (`0010_core_authz.sql`, `0015_core_seed.sql`, `0020_directory_unit_move_jobs.sql` respectively, new numbering). `directory` now precedes `authz`, and `refdata`→`location`→`religion` replaces the old `religion`→`location`→`refdata` order, so every FK is declared inline where the table is created instead of deferred-then-`ALTER TABLE`-closed by a later file — old `0009`↔`0010` and `0011`↔`0012`↔`0014` are the renumbered result (full old→new table in the commit). `0015_core_seed.sql`'s ~25 repeated literal-UUID references (permission grants, edges, closure rows) became `(SELECT id FROM ... WHERE code = ...)` lookups; the six ids that actually get minted still carry their one fixed literal each, per `D-SeedBootstrap` — no seeded value changed, confirmed by diffing `authz_role_permissions` before/after. `citext`/`pg_trgm` moved from the out-of-band `postgres-initdb/01-extensions.sql` init script (deleted) into `0001_registration.sql` alongside `pgcrypto`, now that a from-scratch volume makes the original "Atlas migrations are expand-only, can't retrofit a low-numbered file" constraint moot. `atlas.sum` regenerated; dev volume reset (`docker compose down -v && up --build`) and all 20 migrations applied clean from empty. |
| M12.5 · Admin UI — unit create/edit/archive/delete | ✅ | ✅ | ✅ | ➖ | ✅ | ✅ | **Verified (2026-08-26): real logged-in browser click-through confirmed create-child/edit/archive/search+bulk-archive all work (see M12's own top-level row for the M11.3 session-id regression this pass fixed first); CI green on PR #28, merged to `main`.** Built and live-verified over real HTTP (2026-08-25). Depends on M12.1 (done). Turned `units/[unitId]/page.tsx` from read-only into a full CRUD screen: edit (name/code/level), a create-child-unit form, archive/suspend/reactivate state buttons, and a delete action — all hidden for the root unit (`ancestors.length === 0`, the same signal the existing "This is a root unit" copy already used, no new field needed) since `setUnitState`/`deleteUnit` hard-refuse it regardless of grant. `units/page.tsx` gained a client-side multi-select + bulk-archive action (`bulk-archive-form.tsx`, mirroring M11.7's `bulk-grant-form.tsx` shape but simpler — no secondary search-and-load step) that loops `setUnitState(id, "archived")` per selected unit, deliberately not a new atomic backend bulk endpoint, matching this milestone's own "cheap" framing. **New backend read endpoint**: `unitDeleteEligibility` (`GET /unit-lifecycle/{unitId}/delete-eligibility`, a GET sibling under the existing `/unit-lifecycle/{unitId}` resource — a different HTTP method from the POST routes already there, so none of `updateUnit`/`setUnitState`'s own httprouter wildcard-collision class applies), gated on `unit.lifecycle` over `unitId` — the same authority `deleteUnit` itself requires, since the read reveals `hasActiveRoleAssignments`, otherwise unreadable by anyone without that grant. Factored `DeleteUnit`'s three orphan checks into a shared unexported `checkDeleteEligibility` helper so `DeleteUnit` and the new read can never drift on what counts as blocking. `lib/core.ts` gained wrapper functions for the 5 already-generated-but-previously-unused SDK bindings M12.1 left in place (`createUnit`/`updateUnit`/`setUnitState`/`deleteUnit`) plus the new eligibility read — `make sdk` picked up only the one genuinely new binding. New `UnitStatusBadge` (`Unit.state` is a plain wire string, not a generated enum, unlike every other status badge on this app). Proof: `go build`/`go vet`/`golangci-lint`(`godelw verify`)/`go test ./...` clean; extended `internal/core`'s `TestUnitLifecycleIntegration` against real Postgres with `UnitDeleteEligibility` assertions at every fixture state `DeleteUnit`'s own orphan-protection checks already exercise (children, active role assignments, org profile, root unit, plus the two permission-denial cases) — all pass (the same pre-existing local `identity_audit_log` ownership gap M12.3/M12.4 already documented hit this test's own `t.Cleanup`, unrelated to this milestone); `make sdk`/`sdk-verify` clean; `next build`+`tsc --noEmit`+`eslint` clean for `openfaithmap-admin` (and `tsc --noEmit` clean for `openfaithmap-web` after the shared SDK regen). The rebuilt `openfaithmap-api` container boots clean with no httprouter panic, and the new `delete-eligibility` route correctly `401`s an anonymous caller over the real compose network (HTTPS, port 3001); the rebuilt `openfaithmap-admin` container correctly `307`s an unauthenticated `/admin/units/{unitId}` request to `/login` — routing-level proof only, the same bar M12.1–M12.4 accepted, since this environment still has no dev-issuer key for full `TestAuthorizationMatrix`-style authenticated HTTP and no dev-login bypass for a real browser click-through of the CRUD/bulk-archive forms themselves — left for the user's own browser pass against the now-running `localhost:3004`. |
...
**Depends on:** M11 (the identity/authz/audit-log surface M12's mutations reuse) and M10 (the
`directory`/`authz` core M12 extends). **Leaves deployable:** trivially yes (no code changes — a
docs milestone, same shape as M11 itself). **Blocks:** every M12.x below.

The user asked for a full revision pass on "unit" functionality — a discovery pass on what
comparable systems offer, a discovery pass on what this app already has, then a gap-driven
milestone, before turning anything into code.

**Discovery** (three parallel research/explore agents — one codebase audit, one against
comparable church/ministry management software, one against mature enterprise org-hierarchy/RBAC
platforms) found:

- **Built, and stronger than most comparables**: a real multi-graph hierarchy
  (`directory_graphs`/`directory_unit_edges`/`directory_unit_closure`, materialized and
  incrementally maintained, cycle-guarded), unit/subtree-scoped RBAC
  (`authz_role_assignments.scope`), a separate instance-admin plane, and a PDP with `Explain`/`Via`
  decision tracing — all of which most researched ChMS products (Planning Center, Breeze, CCB,
  MinistryPlatform) don't have, and which mirror patterns from AWS Organizations, Entra
  Administrative Units, and Google Workspace OU-scoped custom roles.
- **Missing entirely, not half-built**: any unit update/delete/archive/suspend/generic-move
  endpoint (`internal/directory` has no HTTP transport at all — `CreateUnit`/`AddEdge`/`RemoveEdge`
  are only reachable in-process); a hierarchy/tree visualization UI (only linear breadcrumbs +
  free-text search exist); an "explain this access decision" UI over the PDP's own existing
  tracing; an invite-with-role-in-one-step flow; bulk unit operations beyond bulk role grant
  (M11.7).
- **A real, live gap** — not cosmetic: two permission codes, `unit.lifecycle` and
  `unit.edges.manage`, are seeded onto real roles (`registration-operator`,
  `platform-moderator`) but never `Require`'d by any code path in the repo — dead permissions
  waiting for the endpoints that would use them.
- **A real, unused column** — not cosmetic: `authz_role_assignments.expires_at` is already
  enforced by the PDP (`ActiveGrantsForSubject` filters on it) but is never written by any grant
  path and never even selected by the admin UI's read queries, so an expired-but-not-revoked
  grant currently renders as a normal active row.

**Scoped jointly with the user**, across two rounds of questions:

- **In scope**: full unit lifecycle CRUD (create beyond `createChildOrg`, update, archive/
  suspend/reactivate, soft-delete with orphan protection), a generic move/reparent capability
  extracted out of `internal/registration`'s private reparent state machine, a real HTTP
  transport for `internal/directory`, audit logging for every unit mutation, admin UI for all of
  the above plus a hierarchy visualization, exposing the PDP's `Explain`/`Via` tracing as an
  admin debugging tool, and wiring up `authz_role_assignments.expires_at`'s write path and
  display (basic version only — see below).
- **Explicitly out of scope**: self-service join/request-to-join-a-unit flows — the user's own
  reasoning, recorded in D-TopDownUnitAccessOnly, is that top-down-only access avoids bottom-up
  privilege-escalation and IDOR/parameter-tampering risk on approval endpoints, and avoids
  approval-fatigue/spam risk, while public read-only unit discoverability stays fully preserved
  and decoupled from authorization. Full PIM-style eligible/active role states, approver
  workflows, and justification capture were considered (enterprise platforms researched — Entra
  PIM, AWS — do this) and dropped as too heavy for this milestone; only the already-enforced
  `expires_at` column's write path and display get fixed, no approval/activation machinery.
  Reintroducing multi-tenancy, first-class unit "kinds," or a canonical fixed-depth hierarchy
  were also ruled out, respecting D-CorePortScope/D-JurisdictionUnits.
- **Move authorization, decided with the user**: D-UnitMoveDualScope — moving a unit requires
  `unit.edges.manage` on *both* the source and destination parent, stricter than the original
  narrow reparent flow, to prevent a one-sided grant from being used to pull a unit into or out
  of a part of the tree the caller has no authority over.
- **Grant-scope prerequisite, decided with the user**: rather than accept that wiring
  `unit.lifecycle`/`unit.edges.manage` to real endpoints instantly hands
  `registration-operator`/`platform-moderator` instance-wide power (their existing seed grants
  are root-subtree-scoped, currently inert since nothing enforces them), the user chose to audit
  and narrow those grants first — M12.0, and `U14` in this doc's unresolved-unknowns table,
  since the correct narrower target needs real-usage data this session didn't have.

> **Correction (M12.0, 2026-08-24): the "root-subtree-scoped, currently inert" framing above was
> half right.** M12.0's audit (see its own stage-board row, and the `D-PlatformModerator` addendum
> in `decisions.md`) found `unit.edges.manage` really is inert exactly as described, but
> `unit.lifecycle` is not — it's live-enforced today as platform-moderator's own identity marker in
> `internal/moderation`/`internal/vouching`, just not yet backing a real unit-lifecycle mutation. The
> fix shipped was a permission split (`moderation.standing`), not a scope narrowing. Separately,
> **"subtree-scoped" was never accurate for either role**: every real/test grant is `scope='unit'`
> (`internal/authz/adapters/store.go` hardcodes it; the grant wire types have no scope field), so
> `unit.edges.manage` at root today reaches only root itself — `registration-operator`'s own scope
> question stays open, deferred to M12.2 (`U14`).

**The two decisions**, in `architecture/decisions.md`: D-UnitMoveDualScope,
D-TopDownUnitAccessOnly. The other six sub-milestones (lifecycle CRUD, move extraction, expiry
write-path, PDP exposure, and the three UI milestones) are additive/corrective work consistent
with the existing architecture and needed no new decision block, the same posture most of
M11.1–M11.9 had.

**Recommended build order**, and why: M12.0 first — a hard gate, since M12.1's `SetUnitState`/
`DeleteUnit` and M12.2's `MoveUnit` must not go live before the two seeded grants are scoped
down. M12.1, M12.3, and M12.4 can then interleave — they touch different code (`directory`
lifecycle, `authz` expiry, `authz` PDP exposure respectively) and have no cross-dependency. M12.2
comes next, deliberately not folded into M12.1: it touches a live production flow
(`internal/registration`'s own approve/reparent path) and is the riskiest backend piece in this
arc, warranting its own focused pass. M12.5–M12.7 (the three UI milestones) follow their
respective backend dependencies (M12.1, M12.2, M12.1) and are otherwise independent of each
other.

Bulk move and multi-graph management UI were both identified as real gaps during discovery but
deliberately deferred to a future M13, not folded in here: bulk move isn't a shallow batch
wrapper like bulk-archive (each unit in a batch can have a different valid source parent, needs
its own cycle check, and would compound the closure-lock contention
`internal/directory/lockcontention_integration_test.go` already exists to guard against at
single-edge granularity), and multi-graph management UI has no existing screen to extend and no
demonstrated user-facing pain point yet — building it speculatively risks the same
"undesigned primitive" pattern D-PlatformModerator's own history warns against.

Each M12.x is its own future execution session, same pattern M10.1–M10.9 and M11.1–M11.9 used —
`Decided`/`Designed` marked now; `Backend`/`Migrated`/`UI`/`Verified` to be filled in as each is
actually built and live-verified.

> **Closing pass (2026-08-26): every M12.x row above is now `✅` Verified, but getting there found two
> real bugs neither this arc's own extensive automated-test suites nor its own routing-level real-HTTP
> checks had caught.** All nine sub-milestones' code had already landed on `feat/m12-unit-management`
> by the end of the prior session; this pass's own job was purely closing the Verified gate: push, PR
> #28, set up a local `DEV_ISSUER_HMAC_KEY`/`docker-compose.override.yml` to run
> `TestAuthorizationMatrix` fully authenticated over real HTTP (closing M12.1's own remaining gap, and
> adding a new `target_scoped_unit_move` case that proves M12.2's dual-scope `moveUnit` gate and
> `grantUnitRole`'s `scope="subtree"` field together the same way), and — since a real Google OAuth
> login is required and this environment has no dev-login bypass — asking the user to click through
> M12.5/M12.6/M12.7's own admin screens with a real logged-in session.
>
> **Bug 1 (found immediately, before the browser pass could get past the first real screen): every
> `web/apps/admin/lib/*.ts` API client except `lib/core.ts` never forwarded `X-Session-Id`.** M11.3
> (D-SessionTracking) made a valid `X-Session-Id` header mandatory alongside the bearer on every
> authenticated request, and `lib/core.ts`'s `client()` was updated to send it — but `registration.ts`,
> `congregation-import.ts`, `content.ts`, `discovery.ts`, `moderation.ts`, and `vouching.ts` were never
> touched to match. Every call through those six files had been silently 401-ing
> (`Identity:Unauthorized`) since M11.3 shipped four days earlier — invisible in `openfaithmap-api`'s
> own access logs, because `internal/identity/middleware.Authenticator.Handle`'s early rejection
> writes the 401 directly and returns before witchcraft's own request-logging middleware ever runs.
> This broke My Congregation, Congregation Import, Registrations, Moderation, and Vouching outright —
> everything except the M12 screens themselves, which are the one part of this app that happened to
> exclusively use the one file that already had the fix. Fixed by mirroring `lib/core.ts`'s own
> `fetch` override in all six files.
>
> **Bug 2 (found by the user's own click-through of M12.6's move form): `moveUnit` orphaned a unit —
> deleted its only parent edge — when `newParentUnitId` equals the unit's own current parent.** Every
> one of M12.2's own integration tests (`TestUnitMoveIntegration`, `TestMoveUnitIntegration`) proved
> the happy path, the dual-scope denial paths, the cycle-rejection path, and the terminal-VERIFIED
> live-slot fix — none of them ever tried moving a unit to the parent it already had, because nothing
> in the milestone's own design review flagged that as a distinct case. `runMoveSteps`'s
> add-before-remove sequence assumes two distinct edges exist while it's mid-move; when old and new
> parent are identical there is only one edge to begin with, so step one ("add the new edge") no-ops
> — the edge already exists — and step two ("remove the old edge") deletes that same edge, leaving the
> unit with zero parents. The job's own verify step correctly noticed the new parent was missing from
> the unit's ancestors afterward and marked itself `FAILED` — the corruption did not go undetected —
> but nothing rolled the deletion back, and a `FAILED` job status reads very differently from "your
> data is now broken" to someone hitting this by accident (the user's own words: "it looks like that
> unit became root unit"). Fixed by rejecting the move upfront in `Move()` itself, before any edge
> mutation runs, with a new `Core:UnitAlreadyAtParent` (CONFLICT) error (`api/core.conjure.yml`,
> mapped in `internal/core/transport/errors.go`, regenerated into both the Go server bindings and the
> TypeScript SDK). Regression cases added directly to both existing suites named above. The one unit
> this bug had already corrupted in this environment's own dev database (a real, pre-existing
> exclusion-list fixture, "Russian Orthodox Church (excluded — D-Exclusions)") was repaired by hand
> (`internal/directory.Service.AddEdge`, the same closure-safe path a real move uses) before the user
> re-tested live: a same-parent move now surfaces a clean "this unit could not be moved" error instead
> of corrupting anything, and a move to a genuinely different parent still succeeds.
>
> Full suite clean afterward: `go build`/`go vet`/`godelw verify`/`go test ./...`, both web apps'
> `lint`+`build`, `sdk-verify`. All 5 CI jobs green on PR #28 across two pushes; merged to `main` as a
> real (non-squash) merge commit, matching the M8–M11 precedent. This is the concrete case for
