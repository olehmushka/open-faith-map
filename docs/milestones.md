...
| M12.4 · PDP `Explain`/`Via` exposure | ✅ | ✅ | ⬜ | ➖ | ⬜ | ⬜ | **Designed, not yet built.** Independent of every other M12.x. Exposes the PDP's existing (test-only-reachable) decision-tracing engine as `explainAccess` on `CoreSuperAdminService` plus a new admin UI page — a "why does this user have this access" debugging tool, matching the role Google Cloud Policy Analyzer / AWS IAM Policy Simulator play in the platforms researched. Instance-admin-only, no audit log entry (pure read). |
