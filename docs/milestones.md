...
| M11.2 · General admin audit log | ✅ | ✅ | ⬜ | ⬜ | ⬜ | ⬜ | **Decided (2026-08-22).** New `identity_audit_log` table (actor, action, target, before/after, timestamp) — migration `0016_core_audit.sql`, one file per module matching the M10.9 collapse convention. A shared logging helper wired into every mutating super-admin path, retrofitted onto the pre-existing role-grant/instance-admin endpoints (M10.8) as well as M11.1's deactivate/reactivate, so the log has no blind spot from day one. UI: an audit log viewer under `(super-admin)`, filterable by actor/target/date, reusing `web/apps/admin/components/data-table.tsx`. Sequenced early, deliberately, so every later milestone in this arc logs against it instead of bolting logging on after the fact. |
| M11.3 · Server-side session tracking + revocation | ✅ | ✅ | ⬜ | ⬜ | ⬜ | ⬜ | **Decided (2026-08-22).** D-SessionTracking — the user chose real session tracking over the lighter status-check-only option. New `identity_sessions` table (session id, account id, issuer, `created_at`, `last_seen_at`, device label, `revoked_at`). NextAuth (`web/apps/admin/auth.ts`) issues a session id into its JWT at sign-in; every backend request checks it's present and unrevoked, alongside the existing token verification. `identity.application` gains `ListSessions`/`RevokeSession` — an admin-scoped variant on `CoreSuperAdminService` and a self-scoped variant for a user's own sessions. UI: a "Sessions" section on the person detail page (admin) and on the self-service profile page (M11.5). Depends on M11.1's enforcement pattern and M11.2's audit log. |
| M11.4 · Last-login / activity tracking | ✅ | ✅ | ⬜ | ➖ | ⬜ | ⬜ | **Decided (2026-08-22).** Derives from M11.3's `identity_sessions.last_seen_at` rather than a duplicate column — updated on each authenticated request or session refresh (decided at build time). UI: "last active" surfaced on the people list/detail pages. Small, sequenced directly after M11.3 since it piggybacks on the new session rows rather than being independent schema; no new migration expected. |
| M11.5 · Self-service profile page | ✅ | ✅ | ⬜ | ➖ | ⬜ | ⬜ | **Decided (2026-08-22).** A self-scoped "update own profile" path in `internal/identity/application` — distinct from the admin-scoped person-update paths, gated only by "caller may touch their own person row," no instance-admin check. UI: replaces `whoami`'s raw-JSON dump (`web/apps/admin/app/[locale]/(chrome)/whoami/page.tsx`) with a real profile page — editable name, read-only role list (reusing `lib/core.ts`'s existing calls), and the "My sessions" panel from M11.3 once that lands. |
| M11.6 · Invite-a-teammate flow (link-based) | ✅ | ✅ | ⬜ | ➖ | ⬜ | ⬜ | **Decided (2026-08-22).** D-InviteLinkMVP — link-based now, real email delivery explicitly deferred (no email infrastructure exists in this repo at all, confirmed by direct grep). Pre-provisions an `identity_persons`/`identity_accounts` row by email/name (reusing the existing insert path, `internal/identity/adapters/store.go:150-153`) and generates a signed one-time invite link the admin copies and shares manually. Must produce a row M10.2's existing JIT link-on-match logic will actually match on first Google sign-in (`IDENTITY_JIT_MATCH=account-email` mode) — the one milestone in this arc that has to be checked against *existing* JIT code, not just new code. UI: an "Invite" action on the people page, showing the generated link to copy. |
| M11.7 · Bulk role assignment | ✅ | ✅ | ⬜ | ➖ | ⬜ | ⬜ | **Decided (2026-08-22).** A batch variant of the existing `GrantUnitRole` (`internal/authz`) taking a list of person ids + one role + unit, in a transaction — no new authorization model, reuses `RequireInstanceAdmin` and the existing per-assignment permission checks. UI: multi-select on the existing people/role-grants pages, "assign role to selected." Logged via M11.2. |
