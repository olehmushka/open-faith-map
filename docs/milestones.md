...
| M12.5 · Admin UI — unit create/edit/archive/delete | ✅ | ✅ | ➖ | ➖ | ⬜ | ⬜ | **Designed, not yet built.** Depends on M12.1. Turns `units/[unitId]/page.tsx` from read-only (its own code comment currently claims no mutation endpoint exists and none belongs there — that comment becomes false) into a full CRUD screen, plus a delete-eligibility read endpoint for UX only and a cheap client-side multi-select bulk-archive action. |
