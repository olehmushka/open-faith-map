...
| U14 | **`platform-moderator`'s half resolved at M12.0** — the audit found `unit.lifecycle` wasn't dead but dual-purposed (real identity marker, not just an inert seed), and the fix was a permission split (`moderation.standing`), not a scope narrowing. **`registration-operator`'s `unit.edges.manage` remains genuinely open**, deferred to M12.2: the audit also found every real/test grant is `scope='unit'`, never `subtree` (`internal/authz/adapters/store.go` hardcodes it, and the wire types have no scope field at all) — so root-scoped `unit.edges.manage` today reaches only the root unit itself, not any descendant, and there is currently no way for registration-operator to be granted reach to an arbitrary jurisdiction parent for the M12.2 `MoveUnit` dual-parent check to ever pass on a non-root target. M12.2 needs to decide: build a way to provision `subtree` grants for real, or design `Reparent`'s refactor around `scope='unit'`-only reach some other way. | `M12.2` |
...
| M12.2 · Generic move/reparent capability | ✅ | ✅ | ⬜ | ⬜ | ➖ | ⬜ | **Designed, not yet built.** Depends on M12.0. Extracts the add-before-remove, resumable, closure-safe move logic currently private to `internal/registration.Service.Reparent` into `internal/directory` as `Move`/`GetMoveStatus`, so registration becomes a caller rather than sole owner. New `directory_unit_move_jobs` table (generalized `jurisdiction_reparenting_jobs`, which is left in place as a frozen historical log per this repo's expand-only migration convention). D-UnitMoveDualScope: `unit.edges.manage` required on both the source and destination parent. The riskiest backend piece — touches a live production flow. |
