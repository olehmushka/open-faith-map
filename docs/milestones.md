...
| M12.3 · Role-assignment expiry (write path + display fix) | ✅ | ✅ | ⬜ | ➖ | ⬜ | ⬜ | **Designed, not yet built.** Independent of M12.0–M12.2. Correction found during design: the PDP already enforces `expires_at` on every decision (`ActiveGrantsForSubject` already filters on it) — what's actually missing is narrower than "add enforcement": `InsertRoleAssignment`/`BulkInsertRoleAssignments` never accept an `expiresAt` argument, and `ListRoleAssignmentsByUnit`/`ListRoleAssignmentsByPerson` don't even select the column, so an expired-but-not-revoked grant currently displays as active in the admin UI. Ships the write path, an expired-badge fix, and a clear-expiry action. No new permission code, no migration. |
