...
| U14 | **What narrower target should replace `registration-operator`'s and `platform-moderator`'s current root-subtree `unit.edges.manage`/`unit.lifecycle` grants** (`migrations/0015_core_seed.sql`)? Both permissions are seeded but currently unenforced, so this is inert today — the user chose to scope the grants down before M12.1/M12.2 wire them to real endpoints, rather than let two roles gain instance-wide unit-lifecycle/edge-management power as a side effect. Needs an audit of how these two roles are actually used before a narrower target (or a role split) can be chosen — not a guess made ahead of that data. | `M12.0` |
...
| M12.0 · Scope down root-subtree grants | ✅ | ✅ | ⬜ | ⬜ | ➖ | ⬜ | **Designed, not yet built.** Prerequisite gate on M12.1/M12.2 — see `U14` above and the M12 detail section. `registration-operator`/`platform-moderator` currently hold `unit.edges.manage`/`unit.lifecycle` root-subtree-scoped but unenforced (`migrations/0015_core_seed.sql`); this audits their real usage and ships a migration narrowing the grants before either permission is wired to a live endpoint. |
