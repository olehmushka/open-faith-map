# Milestones (2026-08-07 – 2026-08-26)

> **Archived, closed record.** Every milestone below (M0–M13.6) is done — no row has unbuilt
> Backend/Migrated/UI work; the handful still showing a blocked or not-yet-CI-verified gate are
> named in their own row's prose, not open build work. Active planning continues in
> [`milestones-2026-09-20-now.md`](milestones-2026-09-20-now.md), by way of the M14–M15 archive
> [`milestones-2026-08-20-2026-09-20.md`](milestones-2026-08-20-2026-09-20.md); this file is kept
> as-is for history and is not updated further.
...
|---|---|---|---|
| U2 | ~~**Can an anonymous caller read `GET /religion/discovery/sites`?** Every `religion` read is `RequireAnywhere`-gated, which denies subjects with no person behind them — and an anonymous caller is one. If the answer is no, the public map cannot read go-oikumenea at all and M4 needs a different design, not an adjustment.~~ **Resolved (2026-08-09, M2.5).** Measured directly: no, an anonymous caller gets a `401` before `RequireAnywhere` is even reached. M4 was designed and built around exactly that answer — lazy cache-only public reads (`discovery_site_cache`), never a direct anonymous call to `religion`. Moot entirely since M10 absorbed the core in-process (no more separate go-oikumenea HTTP hop to deny). *(Correction added 2026-08-26 — this row was left unstruck for over two weeks after its own answer was measured and built on; caught while splitting this file.)* | **M4's `designed` gate** | [M2.5](#m25--discovery-reachability-spike) · [discovery.md](modules/discovery.md) |
| U3 | ~~**Can go-oikumenea re-parent a live `Unit` in the `canonical` graph?** M4.1 has to move every existing congregation under a new jurisdiction unit while preserving its admin's grant. If there is no re-parenting path, that milestone blocks on an upstream request — exactly how M2's own org-creation assumption failed. Check this before scoping the rest of M4.1.~~ **Resolved (2026-08-10, M4.1).** Measured directly against the go-oikumenea checkout: no dedicated reparent endpoint exists, but re-parenting is achievable by composing the generic `tenant` module's `addEdge`/`removeEdge` on the `canonical` graph — a resumable two-call job, not a blocked milestone. Moot entirely since M10 absorbed the core in-process. *(Correction added 2026-08-26, same reason as U2's.)* | **M4.1**, and therefore **M5** | [M4.1](#m41--jurisdiction-units) |
...
| # | The decision | Owner |
|---|---|---|
| U7 | ~~Cross-module foreign keys inside `openfaithmap-api` (`discovery_site_cache.content_site_id` → `content_sites`) are neither permitted nor forbidden by conventions.md.~~ **Resolved (2026-08-26).** Formalized as permitted — every module already shares one Postgres instance/schema since D-SharedDatabase/M10, and the `content_site_id` FK was already built this way; conventions.md now states the rule explicitly. | `DS-OFM-13` |
...
| # | The problem | Where |
|---|---|---|
| U11 | ~~**`churchSiteTypeID` fails silently.** If go-oikumenea's seeded `church` site type is ever renamed, `approveRequest` attaches every congregation to whatever the first site type happens to be, with no error. Prefer failing loudly.~~ **Resolved (2026-08-26).** Both copies (`internal/registration` and its deliberate duplicate in `internal/congregationimport`) now return an explicit error instead of falling back to the first available site type. | [registration.md](modules/registration.md) |
| U12 | ~~**Config bypasses the install-config convention.** `internal/platform/config` exists to hold openfaithmap-api's settings and is empty; `cmd/openfaithmap-api` reads five real settings straight from the environment via `requireEnv` — no schema, no validation, no ECV path for the secrets among them.~~ **Resolved (2026-08-26).** Narrower than originally scoped by the time this was picked up — M10 had already deleted the go-oikumenea-era settings this row once listed. The two settings still bypassing `Install` (`DATABASE_URL`, `GOOGLE_OAUTH_CLIENT_ID`) are now real, schema-validated `config.Install` fields (`database-url`/`google-oauth-client-id` in `var/conf/install.yml`), matching `Environment`'s own M10.2 precedent, each with an explicit boot-time "required" check replacing the old `requireEnv` panic. | [conventions.md](architecture/conventions.md) |
