...
**M15 · Closing the audit-log gap** is built (2026-09-15), promoted from the deferred
`DS-OFM-15` seam: `internal/auditlog` (M11.2) already existed but had only one consumer
(`internal/core`); M15 wires `content`, `registration`, and `congregationimport`'s own real
mutations into the same `Record` primitive, with one named exception
(`congregationimport.RunJurisdictionSync`, coupled to `DS-OFM-16`) — see its own row below.
...
| M15 · Closing the audit-log gap (`DS-OFM-15`) | ✅ | ✅ | ✅ | ➖ | ➖ | ⬜ | **Built (2026-09-15).** `internal/auditlog`'s `Record` (M11.2) had exactly one consumer, `core`, since it shipped — every other module with real mutations (`content`'s site/nav/catalog writes, `registration`'s congregation-approval unit-create/role-grant, `congregationimport`'s alias creation and candidate-approval unit-create) stayed unaudited. No migration: `identity_audit_log`'s `action`/`target_kind` are free text, no DB `CHECK`, so this is wiring, not schema. Each module gets its own private `auditAction*`/`auditTarget*` const block (`core`'s own established convention) and a new `auditLog *auditlogapplication.Service` constructor param, wired through `deps.AuditLogSvc` (new field, populated by `registerCore`, consumed by `registerContent`/`registerRegistration`/`registerCongregationImport`). `PutNavItems` is in scope (a site-settings full-replace, same cluster as theme/chrome); `PutBlocks`/`TransitionDocument` stay out (already covered by M14.6's `content_document_revisions`). One named, deliberately-accepted exception: `congregationimport.RunJurisdictionSync`'s jurisdiction-unit creation keeps writing under `authz.SystemContext` with no audit row, coupled to `DS-OFM-16` (not solved here). New [auditlog.md](modules/auditlog.md). Live-verified against the running docker-compose stack: a real `POST /content/v1/sites` call by a real `congregation-admin` session wrote exactly one `identity_audit_log` row (`CREATE_SITE`, correct actor/target/curated payload). `internal/content/content_integration_test.go`, `internal/registration/registration_integration_test.go`, and `internal/congregationimport/congregationimport_integration_test.go` all gained cases against real Postgres, including negative assertions that `PutBlocks`/`TransitionDocument` add no rows. Opens `DS-OFM-18` (`GrantUnitRole` gives no created-vs-resumed signal, so a resumed registration-approval retry may double-log `GRANT_UNIT_ROLE` — known, low-severity, not fixed here). `Verified` awaits CI green on `main`. |
