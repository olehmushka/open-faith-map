...
func TestEncodeDecodeCursorRoundTrip(t *testing.T) {
	tests := []struct {
		...
		{name: "typical row", cursor: domain.PageCursor{CreatedAt: time.Date(2026, 8, 10, 12, 0, 0, 0, time.UTC), ID: "report-1"}},
		{name: "sub-second precision preserved", cursor: domain.PageCursor{CreatedAt: time.Date(2026, 8, 10, 12, 0, 0, 123456789, time.UTC), ID: "report-2"}},
	}
...
func TestDecodeCursorTamperCases(t *testing.T) {
	tests := []struct {
		...
		{name: "valid JSON, empty createdAt", token: encodeRaw(`{"createdAt":"","id":"report-1"}`)},
		{name: "valid JSON, unparseable createdAt", token: encodeRaw(`{"createdAt":"not-a-date","id":"report-1"}`)},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
